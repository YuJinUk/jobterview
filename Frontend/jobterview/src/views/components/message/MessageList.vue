        ...
        <div class="row justify-content-center1">
            <ul class="nav nav-tabs" id="messageTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="receive-tab" data-bs-toggle="tab" data-bs-target="#receive"
                        ...
                        @click="fetchReceiveMessage(1)">받은 쪽지함</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="send-tab" data-bs-toggle="tab" data-bs-target="#send" type="button"
                        role="tab" aria-controls="send" aria-selected="false" @click="fetchSendMessage(1)">보낸 쪽지함</button>
                </li>
            </ul>
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active mt-2" id="receive" role="tabpanel" aria-labelledby="receive-tab">
                    ...
                    <PagingComponent @changePage="fetchReceiveMessage" :totalPage="totalReceivePage" :currentPage="currentReceivePage"/>
                </div>
                <div class="tab-pane fade mt-2" id="send" role="tabpanel" aria-labelledby="send-tab">
                    ...
                    <PagingComponent @changePage="fetchSendMessage" :totalPage="totalSendPage" :currentPage="currentSendPage"/>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    setup() {
        ...
        let currentReceivePage = computed(() => store.state.messageStore.currentReceivePage);
        let totalReceivePage = computed(() => store.state.messageStore.totalReceivePage);
        let receiveMessage = computed(() => store.state.messageStore.receiveMessage);

        let currentSendPage = computed(() => store.state.messageStore.currentSendPage);
        ...
        async function fetchReceiveMessage(pageNumber) {
            await store.dispatch('messageStore/getReceiveMessages', { nickname: nickname.value, page: pageNumber - 1 });
        }

        async function fetchSendMessage(pageNumber) {
            await store.dispatch('messageStore/getSendMessages', { nickname: nickname.value, page: pageNumber - 1 });
        }
