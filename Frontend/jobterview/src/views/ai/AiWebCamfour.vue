<template>
    ...
    <div id="headTextBox">
        Q4. {{ selectedQuestions[3].content }}
    </div>
    ...
    <div class="main-bg">
      ...
      <div class="clock-wrapper">
        ...
        <div v-else>
            <router-link :to="{ name: 'AiResult' }">
                <div class="next-button-position">
                    <button class="start-button">결과보러가기</button>
                </div>
            </router-link>
        </div>
      </div>
...
methods: {
    ...mapActions('aiStore',['storeResultData', 'storeVideo4']),
    formatTimer(seconds) {
    ...
    if (this.videoData.chunks.length > 0 && this.audioData.length > 0) {
        ...
        try {
            ...
            const response = await axios.post(this.localhost + 'api/upload/4', formData);

            // Flask에서 받은 응답 데이터를 resultData에 저장
            this.resultData = response.data;
            // this.$store.dispatch('aiStore/storeResultData', response.data);
            this.$store.dispatch('aiStore/storeVideo4', response.data);
